...
def benchmark_gradient_descent(D):
    secs = timeit(lambda: gradient_descent(D), number=2) / 2
    print(f"Average time python numpy: {secs}")


def benchmark_gradient_descent_native(D):
    N = D.shape[0]
    D_native = PyMatrix(D.tolist(), N, N)
    secs = timeit(lambda: gradient_descent_native(D_native), number=2) / 2
...
if __name__ == "__main__":
    ...
    ### Without visuals
    p = gradient_descent(D)
    p = gradient_descent_native(D_native)

    
    ### Benchmarks
    benchmark_gradient_descent(D)
    benchmark_gradient_descent_native(D)  
